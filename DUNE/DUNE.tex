...
The design of the DUNE experiment is driven by the physics ambition of the project.  Considerations involving specific experimental details ensure its potential is maximised and the crucial measurements can be made with ease.  This section will briefly report on the current understanding of how DUNE will be planned.  Given the timescale of the project it is likely particular facets will change before construction; the important features which define the design will remain however.
...
A proton beam (60-120~GeV) is extracted from the Fermilab Main Injector and transported through a man-made embankment and bend downwards to establish the final trajectory towards the far detector.  The beam is incident on a target to produce secondary mesons which are subsequently focussed into a decay pipe by magnetic horns where they decay into muons and neutrinos.  The polarity of the horns used determines whether or not the beam is neutrino or antineutrino dominated.  At the end of the decay pipe, the muons and any residual hadrons are stopped to produce a beam of neutrinos, tuned with energies between 0.5~GeV and 5~GeV.  The fluxes of different neutrino species present in the beam in both neutrino- and antineutrino-running modes are shown in Figure~\ref{fig:DUNEBeamFluxes}.  The target is based on the design used in the Fermilab NuMI (Neutrinos from Main Injector) beam, in operation since 2005, and consists of a graphite core with dual titanium water lines to dissipate heat and encased in a titanium containment tube.
...
The STT consists of over 107,000 tubes made from carbon and aluminium with outer diameter 1~cm with double-end readout to provide fine-grained tracking (thickness $<0.1X_0$) with excellent angular and spatial resolution.  The tubes will be filled with either 70\% Ar, 30\% CO$_2$ or 70\% Xe, 30\% CO$_2$ depending on the positioning of the tubes in the detector.  Nuclear targets will be placed upstream using pressurised argon gas or calcium, allowing studies of nuclear effects of neutrino interactions on argon and modelling of signals and backgrounds in the DUNE far detector.  Carbon (graphite) may also be used to study interactions on free protons and statistical subtractions of interactions on H$_2$O and D$_2$O will facilitate analysis of quasi-free neutrons.
...
\end{equation}
inaccessible to water or liquid scintillator detectors which detect the $\bar{\nu}_e$ component via inverse beta decay on free protons, will, along with $\bar{\nu}_e$ interactions, allow precision measurements of the composition of supernovae should one occur during the run of the experiment.  The neutrinos are emitted from a core-collapse supernova in a burst of a few tens of seconds, with around half the luminosity in the first second.  They are roughly divided equally between the three known neutrino flavours and have energies between 5-50~MeV.  A large neutrino signal from a supernova will allow study of astrophysical phenomena related to the end of the lifetime of a star.
...
Each 10~kt module is larger than any LArTPC ever operated by well over an order of magnitude and it is crucial the detector technology is understood and the experiment operates as expected.  It ensure this is the case, a comprehensive prototyping strategy is planned.  This will be briefly overviewed in this section.
